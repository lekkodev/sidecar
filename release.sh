...
docker build -t $registry/$repository:$version --build-arg SIDECAR_VERSION=$version --build-arg SIDECAR_GIT_COMMIT=$git_commit -f Dockerfile.sidecar --platform=linux/amd64 .
